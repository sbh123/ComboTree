...
class BLevel {
 public:
  ...
  Status Insert(uint64_t key, uint64_t value, uint64_t begin, uint64_t end) {
    uint64_t idx = Find_(key, begin, end);
    Status s = GetEntry_(idx)->Insert(&locks_[idx], base_addr_, pop_, clevel_slab_, leaf_node_slab_, key, value);
    if (s == Status::OK) {
      root_->size++;
    }
    return s;
 ...
 private:
  struct Entry {
    ...
    Status Insert(std::shared_mutex* mutex, uint64_t base_addr, pmem::obj::pool_base& pop,
        Slab<CLevel>* clevel_slab, Slab<CLevel::LeafNode>* leaf_node_slab, uint64_t pkey, uint64_t pvalue);
  ...
  Slab<CLevel::LeafNode>* leaf_node_slab_;
