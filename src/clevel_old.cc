...
void CLevel::InitLeaf(MemControl* mem) {
...
Status CLevel::Put(MemControl* mem, uint64_t key, uint64_t value) {
...
Status CLevel::Update(MemControl* mem, uint64_t key, uint64_t value) {
  volatile Node root = root_;
  if (root.IsLeaf())
...
Status CLevel::Get(MemControl* mem, uint64_t key, uint64_t& value) const {
  volatile Node root = root_;
  if (root.IsLeaf())
...
Status CLevel::Delete(MemControl* mem, uint64_t key) {
  volatile Node root = root_;
  if (root.IsLeaf())
...
bool CLevel::Scan(MemControl* mem, uint64_t max_key, size_t max_size,
                  size_t& size, callback_t callback, void* arg) {
  return head_->Scan_(mem, max_key, max_size, size, 0, callback, arg);
}

bool CLevel::Scan(MemControl* mem, uint64_t min_key, uint64_t max_key,
...
CLevel::LeafNode* CLevel::LeafNode::Split_(MemControl* mem, Mutex& mutex,
                                           Node* root) {
...
Status CLevel::LeafNode::Put(MemControl* mem, Mutex& mutex, uint64_t key,
...
Status CLevel::LeafNode::Delete(MemControl* mem, Mutex& mutex, uint64_t key) {
  std::lock_guard<std::mutex> lock(mutex.GetLeafMutex(id));
...
Status CLevel::LeafNode::Update(MemControl* mem, Mutex& mutex,
                                uint64_t key, uint64_t value, Node* root) {
  assert(0);
  return Status::OK;
}

Status CLevel::LeafNode::Get(MemControl* mem, uint64_t key, uint64_t& value) const {
  // lock free
  bool find;
  int sorted_index = Find_(key, find);
  if (!find) {
...
bool CLevel::LeafNode::Scan(MemControl* mem, uint64_t min_key,
...
bool CLevel::LeafNode::Scan_(MemControl* mem, uint64_t max_key,
...
CLevel::IndexNode* CLevel::IndexNode::Split_(MemControl* mem, Node* root) {
...
bool CLevel::IndexNode::InsertChild(MemControl* mem, uint64_t child_key,
...
Status CLevel::IndexNode::Put(MemControl* mem, Mutex& mutex,
                                 uint64_t key, uint64_t value, Node* root) {
  auto leaf = FindLeafNode_(key);
  return leaf->Put(mem, mutex, key, value, root);
}

Status CLevel::IndexNode::Update(MemControl* mem, Mutex& mutex,
                                 uint64_t key, uint64_t value, Node* root) {
  auto leaf = FindLeafNode_(key);
  return leaf->Update(mem, mutex, key, value, root);
}

Status CLevel::IndexNode::Get(MemControl* mem, uint64_t key, uint64_t& value) const {
  auto leaf = FindLeafNode_(key);
  return leaf->Get(mem, key, value);
}

Status CLevel::IndexNode::Delete(MemControl* mem, Mutex& mutex, uint64_t key) {
  auto leaf = FindLeafNode_(key);
  return leaf->Delete(mem, mutex, key);
}

bool CLevel::IndexNode::Scan(MemControl* mem, uint64_t min_key,
                             uint64_t max_key, size_t max_size, size_t& size,
                             callback_t callback, void* arg) {
