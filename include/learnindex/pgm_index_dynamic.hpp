...
#include "pgm_index_nvm.hpp"
#include "pgm_index_old.hpp"
...
public:
    ...
    DynamicPGMIndex() : used_levels(min_level), levels(32 - min_level), pgms() {
        level(min_level).reserve(buffer_max_size + 1);
    ...
    bool put(const K &key, const V &value) { insert(Item(key, value)); return true;}
    bool insert(const K &key, const V &value) { insert(Item(key, value)); return true;}

    /**
     * Removes the specified element from the container.
     * @param key key value of the element to remove
     */
    bool erase(const K &key) { insert(Item(key)); return true;}
    bool remove(const K &key) { insert(Item(key)); return true; };
